...
def parse_rbi_portal():
    ...
    if database["months"]:
        os.makedirs('site', exist_ok=True)  # matches the Cloudflare Pages "Root directory" setting
        with open('site/data.json', 'w') as out:
            json.dump(database, out, indent=2)
        print(f"\n🎉 Ingestion Pipeline Success! Generated structured site/data.json containing {len(database['months'])} periods.")
    else:
