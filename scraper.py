...
# Load the shared column mapping (single source of truth, also used by index.html)
CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'column_mapping.json')
with open(CONFIG_PATH, 'r') as f:
    COLUMN_CONFIG = json.load(f)
COLS = COLUMN_CONFIG['columns']
MIN_ROW_LENGTH = COLUMN_CONFIG['min_row_length']
...
def parse_rbi_portal():
    ...
    # Slice strictly to the latest 12 months
    target_links = unique_links[:12]
    ...
    for idx, item in enumerate(target_links):
        ...
        try:
            ...
            for _, row in df.iterrows():
                if len(row) < MIN_ROW_LENGTH: # Safe margin checks, driven by column_mapping.json
                    continue
                
                c1 = str(row[COLS['sr_no']]).strip() if pd.notna(row[COLS['sr_no']]) else ""
                c2 = str(row[COLS['bank_name']]).strip() if pd.notna(row[COLS['bank_name']]) else ""
                ...
                if c1.isdigit():
                    ...
                    extracted_records.append({
                        ...
                        'cards_outstanding': get_val(COLS['cards_outstanding']),
                        'pos_volume': get_val(COLS['pos_volume']),
                        'pos_value': get_val(COLS['pos_value']),
                        'online_volume': get_val(COLS['online_volume']),
                        'online_value': get_val(COLS['online_value']),
                        'others_volume': get_val(COLS['others_volume']),
                        'others_value': get_val(COLS['others_value']),
                        'atm_volume': get_val(COLS['atm_volume']),
                        'atm_value': get_val(COLS['atm_value'])
                    })
    ...
    if database["months"]:
        # Stamp with a UTC generation timestamp so the dashboard can display real
        # pipeline freshness ("data refreshed X ago") instead of just the client's
        # own page-load time, which is misleading for a public-facing trust signal.
        database["generated_at"] = datetime.utcnow().isoformat(timespec='seconds') + "Z"
